...
virtual bool VelocityRampSmoother::getPathBetweenWaypoints(
  ...
  // TODO(helenol): move to timing utils.
  int64_t dt_ns = secToNSec(sampling_dt_);
  ...
  for (size_t i = 0; i < num_elements; ++i) {
    ...
    if (total_segment_time < min_acceleration_time * 2) {
      if (current_time < total_segment_time / 2.0) {
        velocity += constraints_.a_max * sampling_dt_;
      } else {
        velocity -= constraints_.a_max * sampling_dt_;
      }
    } else {
